...
public class Grid<TGridObject>
{
    ...
    private void ShowDebug(int width, int height)
    {
        ...
        for (int x = 0; x < _gridArray.GetLength(0); x++)
        {
            for (int y = 0; y < _gridArray.GetLength(1); y++)
            {
                debugTexArray[x, y] = UtilsClass.CreateWorldText(_gridArray[x, y]?.ToString(), null, GetWorldPosition(x, y) + new Vector3(_cellSize, _cellSize) * .5f, 20, Color.white, TextAnchor.MiddleCenter);
